...
def rulesHook(oNode):
    if not oNode['parent'].has_key('rules'):
      oNode['parent']['rules'] = []
...
def rule_directiveHook(oNode):
    if not oNode['parent'].has_key('rule_directive'):
