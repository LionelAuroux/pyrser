...
""", "Bad pretty printing of type")
    ...
    def test_03_overload(self):
        ...
        self.assertIn(Signature('f', 'int', 'int', 'double', 'char'), trestf, "Bad get_by_params in type_checking.Set")
        self.assertEqual(len(trestf), 1, "Bad get_by_params in type_checking.Set")
        self.assertEqual(len(trestp), 3, "Bad get_by_params in type_checking.Set")
        a = trestp.get_by_symbol_name('a')
        self.assertEqual(len(a), 1, "Bad get_by_params in type_checking.Set")
        sa = next(iter(a.values()))
        self.assertEqual(sa.tret, "int", "Bad get_by_params in type_checking.Set")
        b = trestp.get_by_symbol_name('b')
        self.assertEqual(len(b), 1, "Bad get_by_params in type_checking.Set")
        sb = next(iter(b.values()))
        self.assertEqual(sb.tret, "double", "Bad get_by_params in type_checking.Set")
        c = trestp.get_by_symbol_name('c')
        self.assertEqual(len(c), 1, "Bad get_by_params in type_checking.Set")
        sc = next(iter(c.values()))
        self.assertEqual(sc.tret, "char", "Bad get_by_params in type_checking.Set")
